...
patches-own [
  ...
  delivery-zone ; Zone de livraison : "drone" ou "vehicle"
]
...
to setup-patches
  ask patches [
    ...
    set traffic-level random 4 ; Densité de trafic aléatoire entre 0 et 3

    ; Définir les zones de livraison (drone ou véhicule)
    let distance-to-center distancexy -3.5 -16.5
    if distance-to-center <= 10 [
      set delivery-zone "drone"
    ]
    if distance-to-center > 10 [
      set delivery-zone "vehicle"
    ]
  ]
end
...
to setup-distribution-center
  create-turtles 1 [
    setxy -3.5 -16.5            ; Placer au centre
    set shape "building store"          ; Forme spécifique
    set color yellow            ; Colorer en jaune
    set size 2                  ; Agrandir la taille
  ]
...
to go
  ...
  ask vehicles [
    ...
    ; Vérifier la proximité des clients
     let nearby-clients patches in-radius 5 with [terrain-type = "client"]
  if any? nearby-clients [
    ask nearby-clients [
      set pcolor  ; Changer la couleur en vert
    ]
  ]
  ]
  ...
  ask drones [
    ...
    if [terrain-type] of target = "client" and [delivery-zone] of target = "drone" [
      face target
      fd 1
    ]
  ]
